...
class KDNode {
 ...
 protected static void nnbr(KDNode kd, HPoint target, HRect hr, double max_dist_sqd, int lev, int K,
         NearestNeighborList nnl, double alpha) {
     ...
     nnbr(nearer_kd, target, nearer_hr, max_dist_sqd, lev + 1, K, nnl, alpha);
     ...
     if (HPoint.eucdist(closest, target) < Math.sqrt(max_dist_sqd) / alpha) {
         ...
         nnbr(further_kd, target, further_hr, max_dist_sqd, lev + 1, K, nnl, alpha);
